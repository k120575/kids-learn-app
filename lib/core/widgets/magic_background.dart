...
class _MagicPainter extends CustomPainter {
  ...
  void _candle(Canvas c, Offset o, double r, double a) {
    c.drawCircle(
      o,
      r * 2.4,
      Paint()..color = const Color(0xFFFFE082).withValues(alpha: a * 0.25),
    );
    c.drawCircle(
      o,
      r,
      Paint()..color = const Color(0xFFFFF3C0).withValues(alpha: a),
    );
  }
  ...
  void _broomRider(Canvas c, Offset o, double s, double p) {
    ...
    for (int i = -3; i <= 3; i++) {
      c.drawLine(
        Offset(-s * 1.4, s * 0.15),
        Offset(-s * 2.4, s * 0.15 + i * s * 0.12 + sway),
        br,
      );
    }
    // 斗篷飄動
  ...
  void _castleScene(Canvas c, double w, double h, Random rng) {
    ...
    c.drawRect(
      Rect.fromLTWH(cx - w * 0.17, wallTop, w * 0.34, h * 0.15),

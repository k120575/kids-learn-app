...
class AppDb {
  ...
  Future<void> _upgrade(Database db, int oldVersion, int newVersion) async {
    if (oldVersion < 2) {
      ...
      await db.execute(
          'CREATE TABLE stars(profile_id TEXT NOT NULL, game_id TEXT NOT NULL, '
          'stars INTEGER NOT NULL, PRIMARY KEY(profile_id, game_id))');
      await db.execute("INSERT INTO stars(profile_id, game_id, stars) "
      ...
      await db.execute(
          'CREATE TABLE stickers(profile_id TEXT NOT NULL, sticker TEXT NOT NULL, '
          'earned_at INTEGER NOT NULL, PRIMARY KEY(profile_id, sticker))');
      await db.execute("INSERT INTO stickers(profile_id, sticker, earned_at) "
  ...
  Future<void> _seedDefaultProfile(Database db) async {
    ...
    await db.insert(
      'settings',
      <String, Object?>{'key': 'active_profile', 'value': kDefaultProfile},
      conflictAlgorithm: ConflictAlgorithm.ignore,
    );
  }
  ...
  Future<void> upsertProfile(
          String id, String name, String emoji, int createdAt, int sort) =>
      _db!.insert(
        'profiles',
        <String, Object?>{
          'id': id,
  ...
  Future<Map<String, int>> loadStars(String profileId) async {
    final List<Map<String, Object?>> rows = await _db!.query('stars',
  ...
  Future<void> setStar(String profileId, String gameId, int stars) => _db!.insert(
        'stars',
        <String, Object?>{
  ...
  Future<List<String>> loadStickers(String profileId) async {
    final List<Map<String, Object?>> rows = await _db!.query('stickers',
        where: 'profile_id = ?',
  ...
  Future<Map<String, int>> loadDifficulty(String profileId) async {
    final List<Map<String, Object?>> rows = await _db!.query('difficulty',
        where: 'profile_id = ?', whereArgs: <Object?>[profileId]);
    return <String, int>{
      for (final Map<String, Object?> r in rows)
  ...
  // ----- 遊玩紀錄（家長報告用）-----
  Future<void> addPlay(String profileId, String gameId, int stars,
          int mistakes, int playedAt) =>
  ...
  Future<List<Map<String, Object?>>> gameStats(String profileId) => _db!.rawQuery(
  ...
  Future<Map<String, int>> loadDailyTime(String profileId) async {
    final List<Map<String, Object?>> rows = await _db!.query('daily_time',
        where: 'profile_id = ?',
        whereArgs: <Object?>[profileId],
        orderBy: 'day DESC',
        limit: 14);
  ...
  Future<(int, int)> loadWallet(String profileId) async {
    final List<Map<String, Object?>> rows = await _db!.query('wallet',
        where: 'profile_id = ?', whereArgs: <Object?>[profileId], limit: 1);
    if (rows.isEmpty) {
  ...
  Future<Map<String, int>> loadToys(String profileId) async {
    final List<Map<String, Object?>> rows = await _db!.query('toys',
        where: 'profile_id = ?', whereArgs: <Object?>[profileId]);
    return <String, int>{
      for (final Map<String, Object?> r in rows)
  ...
  Future<void> setToy(String profileId, String toyId, int count, int earnedAt) =>
      _db!.insert(
        'toys',
        <String, Object?>{
          'profile_id': profileId,
  ...
  Future<Map<String, int>> loadAchievements(String profileId) async {
    final List<Map<String, Object?>> rows = await _db!.query('achievements',
        where: 'profile_id = ?', whereArgs: <Object?>[profileId]);
    return <String, int>{
      for (final Map<String, Object?> r in rows)
  ...
  Future<void> setAchievement(
          String profileId, String achId, int tier, int unlockedAt) =>
      _db!.insert(
        'achievements',
        <String, Object?>{
          'profile_id': profileId,
  ...
  Future<(String, int, int)?> loadStreak(String profileId) async {
    final List<Map<String, Object?>> rows = await _db!.query('streak',
        where: 'profile_id = ?', whereArgs: <Object?>[profileId], limit: 1);
  ...
  Future<void> setStreak(
          String profileId, String lastDay, int current, int best) =>
      _db!.insert(
        'streak',
        <String, Object?>{
          'profile_id': profileId,

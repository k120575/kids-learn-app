...
你的開發者帳號（`<你的開發者 email>`）是**個人帳號、且在 2023/11/13 之後新建**，Google 強制規定：
...
> （若 `<你的開發者 email>` 已註冊過開發者帳號，跳過本階段。）

1. 開 **https://play.google.com/console/signup**，用 `<你的開發者 email>` 登入
2. 帳號類型選 **Personal**（個人）
...
2. **Email addresses** 加 `<你的開發者 email>`（及任何要測購買的帳號）
...
- **我們是誰**：寶貝學習樂園，開發者 Kevin，聯絡 `<你的開發者 email>`

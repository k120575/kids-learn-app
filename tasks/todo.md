...
- [ ] **音樂域補完（本 session 最大未完成項）**：每個年齡段音樂目前只有 **1 個**（3-4/4-5 樂器配對、5-6 音波記憶），要各補到 3（各 +2）。已定案、依發展研究排序（**音色/力度/速度→早、音高推理→晚**；音樂能力非 3/3/3 均勻分布）：
  - 3-4：**大聲小聲**（力度二分）、**高高低低**（高/低，**限大音程**：鳥🐦vs象🐘 差八度以上才適齡，細微音高方向是 6 歲技能）
  - 4-5：**快快慢慢**（速度二分，可做漸進）、**音的長短**（ta/ti-ti，Orff 節奏原語，非 pitch）
  - 5-6：**音往哪裡走**（pitch direction 上行/下行——研究指此為 6 歲技能，放這裡才對，原本誤排 4-5）、**哪個音不對**（熟悉小調中找走音）
  - 技術：用 `tool/gen_sfx.py`（ffmpeg sine）合成不同**音高/速度/力度/長短**的 tone clip → `assets/sfx/`；UI 沿用 PickGame/音波記憶的「聽完→點選/拖曳」架構＋適性難度。
  - ⚠️ **避開節奏「回放/跟拍」**：節奏跟打/節奏進階已因音訊時序問題移除過，風險高，不要重做。
  - ⚠️ 收尾鐵則：新關卡標題（大聲小聲/高高低低/快快慢慢/音的長短/音往哪裡走/哪個音不對）要確認在 title 字型 subset 內，否則重跑 `subset_title_font.py`；新台詞加進 `gen_voice.py` 重烤。
  - 建議從 3-4「高高低低」做範本，驗證「合成音高 tone→烤檔→點選」管線通了再補其餘 5 個。
